<?php define ("SERIAL_DEVICE_NOTSET", 0); define ("SERIAL_DEVICE_SET", 1); define 
...
E_USER_WARNING);
	...
	function confBaudRate ($rate)
	{
		...
		{
			trigger_error("Unable to set the baud rate : the device is either not 
...
$parity{0}, $out);
	...
	function confCharacterLength ($int)
	{
		if ($this->_dState !== SERIAL_DEVICE_SET)
		{
			trigger_error("Unable to set length of a character : the device is 
...
$out);
	...
	function confStopBits ($length)
	{
		if ($this->_dState !== SERIAL_DEVICE_SET)
		{
			trigger_error("Unable to set the length of a stop bit : the device is 
either not set or opened", E_USER_WARNING);
			return false;
		}
...
$out);
	...
	function confFlowControl ($mode)
	{
		if ($this->_dState !== SERIAL_DEVICE_SET)
		{
			trigger_error("Unable to set flow control mode : the device is either 
