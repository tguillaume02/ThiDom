...
{	  
	...
	$req = execute_sql("SELECT Lieux.Nom as Lieux, Device.Nom as DeviceName, date_format(cmd_device.DATE, '%Y-%m-%d %H:%i') as DATE,Value as temp, Type_Device.widget, cmd_device.ID as cmd_device_id, Device.id  AS id  from Type_Device 
	...
	While ($donnees = $req->fetch_array())
	{
		$date = $donnees["DATE"];
		$temp = $donnees["temp"];
		if (is_numeric($temp) && $date != "0000-00-00 00:00")
		{			
			$row_array["temp"] = $temp;
			$row_array["date"] = $date;
		}
		...
		$row_array["device_id"] = $donnees["id"];	

		$diff_temp = execute_sql(" SELECT temp
					 FROM Temperature_Temp
					 WHERE lieux = '".$donnees["Lieux"]."' and Cmddevice_Id = '".$donnees["cmd_device_id"]."'
...
if ($act == "Etat"){
	...
	$req = execute_sql("SELECT Device.id  AS id,cmd_device.id as cmd_device_id, cmd_device.Nom as Nom, Type as Type , Lieux.Nom as Lieux, cmd_device.Value as Value, cmd_device.Etat as Etat, cmd_device.Date as Date, Type_Device.widget as Widget 
						from Device 
						inner join cmd_device on cmd_device.Device_ID = Device.ID 
...
if ($mode == "valide")
{
	...
	/*	$req = $mysqli->query("SELECT NOM FROM Etat_IO WHERE id = '$id'") or die('Erreur SQL !<br>');		
		...
		{		
			...
			{	
				$value = array();
				$msg = $app_name." a bien �t� ajout� dans ".$piece_name;
		...
		else
		{
			...
			/*$value = array();
			$msg = "Ce nom d'appareil existe d�j�, veuillez renomer cette appareil";
	...
	elseif ($property == "delete_app")
	{
		...
		{	
			$value = array();
			$msg = $app_name." a bien �t� supprim� de ".$piece_name;
			$value[] =  Array( "msg"=>$msg,"clear"=>"on");
			echo json_encode($value);
		}		
	}
	...
	elseif ($property == "add_piece")
	{
		if ($id != "")
		{
			...
			{	
				$value = array();
				$msg = "La pi�ce ".$piece_name." a bien �t� ajout�e";
				$value[] =  Array( "msg"=>$msg,"clear"=>"on");
				echo json_encode($value);
			}	
		}
	...
	{		
		...
		{
			$value = array();
			$msg = "Veuillez selectionner une pi�ce ";
			$value[] =  Array( "msg"=>$msg,"clear"=>"on");
			echo json_encode($value);
		}
		elseif ($nb == 0)
		{
			...
			{	
				$value = array();
				$msg = "La pi�ce ".$piece_name." a bien �t� supprim�e";
				$value[] =  Array( "msg"=>$msg,"clear"=>"on");
				echo json_encode($value);
			}
		}
		...
		{
			$value = array();
			$msg = "Impossible de supprimer cette pi�ce, supprimez tout d'abord les �l�ments contenus dans celle-ci ";
			$value[] =  Array( "msg"=>$msg,"clear"=>"on");
			echo json_encode($value);			
		}
	}
	...
	{		
		if ($user_pass == $user_pass2)
		{
			...
			if ( $id == "")
			{
				$req = execute_sql("INSERT INTO User (USERNAME, USERPASS) values('$user_name', '$pass')");
				if($req == TRUE)
				{	
					$value = array();
					$msg = "User ajout�";
	...
	elseif($property == "add_plugins")
	{
		if ($app_name != "" && $piece_name != "" && $Request !="")
		{
			...
			foreach ($lines as $lineNumber => $lineContent)
			{
				...
				$req1 = execute_sql("INSERT INTO cmd_device (Nom,Device_ID,sensor_attachID,Request,Value,Etat,RAZ,Visible) select '$InstallArray[0]', MAX(ID) ,-1,'$InstallArray[1]',0,0,'$InstallArray[2]',1 from Device");
			}

			if($req == TRUE and $req1 == TRUE)
			{	
				$value = array();
				$msg = "Le plugins ".$app_name." a bien �t� ajout�";
				$value[] =  Array( "msg"=>$msg,"clear"=>"on");
				echo json_encode($value);		
			}
		}
...
{	
	...
	$requete = execute_sql("SELECT ID,DAYS,HOURS,STATUS,ACTIVATE from planning where Cmddevice_Id='".$id."' order by DAYS,HOURS");
	$JSON = array();
	$row_array["ID"] ="SELECT ID,DAYS,HOURS,STATUS,ACTIVATE from planning where Cmddevice_Id ='".$id."'";
	while ($data = $requete->fetch_array())
	{
		$Days = "";
...
{	
	...
	if ($days <> "" and $hours <> "" and $commande <> "" )
	{
		if ($planningid <> "")
		{
			$req = execute_sql("UPDATE planning set Cmddevice_Id = $deviceid, DAYS = '$days', HOURS = '$hours', STATUS = '$commande', ACTIVATE = '$active' where ID = $planningid") ;
		}
		else
		{
			$req = execute_sql("INSERT INTO planning (Cmddevice_Id,DAYS,HOURS,STATUS,ACTIVATE) VALUES ($deviceid,'$days','$hours','$commande','$active')") ;
		}
		
		if($req == TRUE)
		{	
			$value = array();
			$msg = "Ajout effectu� au planning";
			$value[] =  Array( "msg"=>$msg,"clear"=>"on");
			echo json_encode($value);		
		}
	}
		    ...
		    foreach ($value as $k => $v) {
			    ...
			    if ($conditions."" != "" and $actions."" != "")
			    {
			    	...
			    	{		
						...
						$req = execute_sql("UPDATE Scenario_Xml set XML = '$Xml_Scenario' ,Status = $Xml_Status, Name='Scenario_Name' where ID = '$Scenario_id' ") ;
					}
			    }
...
if ($mode == "LoadPlugins")
{
	$req =  execute_sql("SELECT Device_Id, Request from cmd_device where Request is not null and Request !=''  order by Device_Id");
	$liste_plugins = "|";
  $old_Device_Id =  "" ;
	While ($donnees =  $req->fetch_array())
	{
   if ($old_Device_Id != json_decode($donnees["Device_Id"]))
   {
		$parsed_json = json_decode($donnees["Request"]);
		$liste_plugins .= $parsed_json->{'url'};		
		$liste_plugins .= "|";
   }
   $old_Device_Id =  json_decode($donnees["Device_Id"]);
	}
	...
	{ 
		...
		{	 
			if($fichier != '.' && $fichier != '..' && $fichier != 'index.php' && stripos($liste_plugins, "|plugins/" . $fichier.'/Desktop/'.$fichier.".php|") === false)
			{
				$plugins_array[] = Array( "Nom"=>str_replace('.php','',$fichier),"url"=>"plugins/" . $fichier.'/Desktop/'.$fichier.'.php');
			}
		}
...
if ($mode == "LoadConfigPlugins")
{
	$req = execute_sql("SELECT Configuration from Device where ID ='$id'");
	While ($donnees =  $req->fetch_array())
	{
		/*$parsed_json = json_decode($donnees["Configuration"]);
		$liste_ConfigPlugins .= $parsed_json->{'url'};		
		$liste_ConfigPlugins .= "|";
		$plugins_array = array();
		echo json_encode($plugins_array);*/
		echo $donnees["Configuration"];
	}
}
