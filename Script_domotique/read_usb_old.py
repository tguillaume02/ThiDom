					...
					#Action(value,status,pinID,SlaveCarteId,str_usb_read)					
						...
						try:
							...
							try:
								...
								data = {}
								data['id'] = 'thibault'
								data['notif'] = "Erreur dans la requete"
								data['id_notif'] = "-2"
								url_values = urllib.urlencode(data)
								full_url = url + '?' + url_values
								urllib2.urlopen(full_url)
...
def Temp(pinID,value):
	...
	if pinID == 'Exterieur':
		...
		if min_diff >=30:
			...
			if Bnotif == True:
				try:
					date = time.strftime('%y-%m-%d %H:%M:%S',time.localtime())
					cursor.execute("INSERT INTO Log (DeviceID, DATE,ACTION, Message) VALUES (%s, %s, %s, %s)", (Etat_IO_ID, date,'', str_usb_read))
	...
	if Etat_IO_ID != -1:
		cursor.execute("INSERT INTO Temperature_Temp VALUES (%s, %s, %s, %s)", (date,value,lieux, Etat_IO_ID))
