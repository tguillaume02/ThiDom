...
while True:	
	try:
		...
		sql_alert = " select ID from ( select ID,Etat,Date,Date_SUB(now(), INTERVAL RAZ MINUTE) as raz  from Etat_IO  ) as t where t.Date< t.raz and t.Etat = 1 "		
		cursor.execute(sql_alert)
		for row in cursor.fetchall():
			DeviceID = row[0]
			cursor.execute(" update Etat_IO set Etat_IO.Etat ='0' where ID ="+str(DeviceID))				
